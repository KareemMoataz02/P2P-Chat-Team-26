    ...
    def run(self):
        ...
        if self.peerServer.isChatRequested == 0 and self.responseReceived is None and self.room != 1:
            ...
            if self.responseReceived[0] == "OK":
                ...
                self.peerServer.p = self.responseReceived[1]
        ...
        while choice != "3":
            ...
            # and the password to login
                ...
                if portUsed:
                    ...
                    continue
                # portUsed = check_port.connect_ex((self.registryName, roomServerPort)) == 0
                # if portUsed:
                #     cmsg.red_message("Room port is already in use...")
                #     continue
            ...
            # for a username that is wanted to be searched
                ...
                if searchStatus is not None and searchStatus != 0:
                    cmsg.blue_message("IP address of " +
            ...
            # to enter the username of the user that is wanted to be chatted
                ...
                if searchStatus == self.peerServer.peerServerHostname + ":" + str(self.peerServer.peerServerPort):
                    ...
                    self.peerClient = PeerClient(searchStatus[0], int(
                        searchStatus[1]), self.loginCredentials[0], self.peerServer, None)
                    self.peerClient.start()
                    self.peerClient.join()
            elif choice == "6" and self.isOnline:
                while True:
                    roomid = input("Enter room name (q to exit): ")
                    if roomid == "q":
                        break
                    elif len(roomid) < 3:
                        cmsg.red_message(
            ...
            # main process waits for the client thread to finish its chat
                ...
                self.peerClient = PeerClient(
                    self.peerServer.connectedPeerIP, self.peerServer.connectedPeerPort, self.loginCredentials[0], self.peerServer, "OK")
                self.peerClient.start()
                self.peerClient.join()
